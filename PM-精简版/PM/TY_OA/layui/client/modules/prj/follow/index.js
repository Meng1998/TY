...
layui.extend({
    ...
    */
    ...
    element.on("tab(LAY-project-manage)", function (data) {
        //第一次加载
        if (!tableInsTotal) {
            tableInsTotal = table.load({
                elem: LAY_TOTAL_PROJECT_MANAGE,
                url: "/prj/info/list",
                height: "full-150",
                toolbarEvent: toolbarEvent,
                cols: [
                    ...
                    { title: "跟进状态", field: "follow_status", align: "center", width: 100, templet: function (row) { return FollowStatus.Formatter(row.follow_status); } },
                    ...
                    { title: "操作", width: 100, align: "center", fixed: "right", toolbar: "#total-tool" }
                ],
                fixed: { project_status: ProjectStatus.follow }
            });
        }
    });
    ...
    //监听搜索
    form.on("submit(LAY-total-project-search)", function (data) {
        tableInsTotal.search(data);
        return false;
    });

    //监听重置
    form.on("submit(LAY-total-project-reset)", function (data) {
        tableInsTotal.reset(data);
    });
